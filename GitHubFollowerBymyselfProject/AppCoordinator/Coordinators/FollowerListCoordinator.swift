...
final class FollowerListCoordinator: Coordinator {
    ...
    func start() {
        let module = FollowerListBuilder.build(userName: userName,
                                               container: container)
    ...
    func showFollowerDetail(of follower: Follower) {
        let coordinaotr = FollowerDetailCoordinaotr(navigationController: navigationController, container: container, follower: follower)
